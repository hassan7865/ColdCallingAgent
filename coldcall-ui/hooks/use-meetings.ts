...
import { meetingsKeys } from "@/lib/query-keys";

const api = createResourceApi<Record<string, unknown>, Record<string, unknown>, Record<string, unknown>>("/meetings");

export function useGetMeetings(params: Record<string, unknown> = {}) {
  return useQuery({ queryKey: meetingsKeys.list(params), queryFn: () => api.list(params) });
}
...
export function useUpdateMeetings() {
  const queryClient = useQueryClient();
  return useMutation({
