...
import { callsKeys } from "@/lib/query-keys";

const api = createResourceApi<Record<string, unknown>, Record<string, unknown>, Record<string, unknown>>("/calls");

export function useGetCalls(params: Record<string, unknown> = {}) {
  return useQuery({ queryKey: callsKeys.list(params), queryFn: () => api.list(params) });
}
...
export function useCreateCalls() {
  ...
  return useMutation({
    ...
    onSettled: async (_data, error) => {
      await queryClient.invalidateQueries({ queryKey: callsKeys.all });
...
export function useUpdateCalls() {
  const queryClient = useQueryClient();
  return useMutation({
