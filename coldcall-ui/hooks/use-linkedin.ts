...
import { linkedinKeys } from "@/lib/query-keys";

const api = createResourceApi<Record<string, unknown>, Record<string, unknown>, Record<string, unknown>>("/linkedin/messages");

export function useGetLinkedin(params: Record<string, unknown> = {}) {
  return useQuery({ queryKey: linkedinKeys.list(params), queryFn: () => api.list(params) });
}

export function useCreateLinkedin() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (payload: Record<string, unknown>) => api.create(payload),
    onSettled: async (_data, error) => {
      await queryClient.invalidateQueries({ queryKey: linkedinKeys.all });
...
export function useDeleteLinkedin() {
  const queryClient = useQueryClient();
  return useMutation({
